"""RSI 14 win-rate matrix on short horizons.

Run:  python research/rsi/matrix.py
"""

import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent))

import numpy as np

from research.framework import load_data, compute_matrix, write_xlsx

OUT_DIR = Path(__file__).parent / 'output'
HORIZONS = list(range(1, 31))   # +1d, +2d, ... +30d
N_THRESHOLDS = 30

OUT_DIR.mkdir(exist_ok=True)
data = load_data()
feat = data['rsi_14'].dropna()
lo = np.nanpercentile(feat, 2)
hi = np.nanpercentile(feat, 98)
thresholds = np.linspace(lo, hi, N_THRESHOLDS)

print(f'rsi_14  range [{lo:.4g}, {hi:.4g}]')

p_above, p_below = compute_matrix(data['rsi_14'], thresholds, HORIZONS, data)
write_xlsx(p_above, p_below, OUT_DIR / 'matrix.xlsx')
